...
bool display( vector<float> const & measures,
              vector<float> const & filtered,
              ...
              IplImage & pict )
{
    ...
    {
        cvDrawCircle(&pict, cvPoint(*tgt, *(tgt++)), *(w++) * scale, cvScalar(200,0,200),2);
    }
    ...
    for(float angle = CV_PI/2 - 0.03f;; angle += 0.01)
    {
        ...
        for (unsigned int i=0; i< n_targets; ++i)
        {
            ...
            if( isTargetVisible(detectionProbability) )
            {
                ...
                targetMeasSpeed.push_back( 0.f /* measurements[0] - previousPoses[i].first */);
                targetMeasSpeed.push_back( 0.f /* measurements[1] - previousPoses[i].second */);
        ...
        // Show our drawing
        if(!display( targetMeasPosition, targetEstimPosition, targetEstimWeight, *image ))
        {
            break;
        }
    }
